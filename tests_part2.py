...
class HW4Part2Tests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.opstack_output = {
            ...
            'test33': [DictionaryValue({1: StringValue('(supersuperlongwordletseeifitworksorifithasamaximumlimitofcharctersitcanhandlebeoforeitdoesentwork)'), 2: StringValue('(x)')})],
            'test34': [4, 2, 5], 
            'test35': [10, 30000, 2], 
            'test36': [120, 201, DictionaryValue({'/x': 120, '/y': 201})], 
            'test37': [StringValue('(test)'), True, 8]} 
    ...
    def test_input1(self):
        ...
        self.assertEqual(len(self.ps_env.opstack),
        ...
        for i in range(0, len(self.opstack_output[test_case])):
            self.assertTrue(self.compareObjectData(
                self.ps_env.opstack[i], self.opstack_output[test_case][i]))
...
# MY ADDITIONAL TEST 1

    def test_input33(self):
        testinput33 = """
            /myd 1 dict def
            myd
            myd 1 (supersuperlongwordletseeifitworksorifithasamaximumlimitofcharctersitcanhandlebeoforeitdoesentwork) put
            myd 2 (x) put
        """
...
# MY ADDITIONAL TEST 5

    def test_input37(self):
        testinput37 = """
            (test) dup /str exch def 
            (test) str eq
            /x 4 def
            /y 2 def
            y x mul
